...
from tcms.settings.common import JIRA_PASSWORD, JIRA_USERNAME
...
class JIRA(IssueTrackerType):
    ...
    user_name = JIRA_USERNAME # if self.bug_system.api_username=="" else self.bug_system.api_username
    password = JIRA_PASSWORD # if self.bug_system.api_password=="" else self.bug_system.api_password

    def _rpc_connection(self):
        if self.bug_system.api_username is None or self.bug_system.api_password is None:
            self.user_name = JIRA_USERNAME
            self.password = JIRA_PASSWORD
        else:
            self.user_name = self.bug_system.api_username
            self.password = self.bug_system.api_password
        ...
        return jira.JIRA(
            ...
            basic_auth=(self.user_name, self.password),
            options=options,
        )
    ...
    def report_issue_from_testexecution(self, execution, user):
        ...
        try:
            ...
            new_issue.update(reporter={'name':user.get_full_name()})
